...
!(function($) {
  ...
  $(window).on('scroll', function() {
    ...
    nav_sections.each(function() {
      ...
      if (cur_pos >= top && cur_pos <= bottom) {
        if (cur_pos <= bottom) {
          main_nav.find('li').removeclassName('active');
        }
        main_nav.find('a[href="#' + $(this).attr('id') + '"]').parent('li').addclassName('active');
      }
      if (cur_pos < 300) {
        $(".nav-menu ul:first li:first, .mobile-nav ul:first li:first").addclassName('active');
      }
    });
  });
  ...
  $(window).scroll(function() {
    if ($(this).scrollTop() > 100) {
      ...
      $('#topbar').addclassName('topbar-scrolled');
    } else {
      $('#header').removeclassName('header-scrolled');
      $('#topbar').removeclassName('topbar-scrolled');
    }
  });
